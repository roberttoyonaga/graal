...
public class CustomInlineBeforeAnalysisGraphDecoderImpl extends InlineBeforeAnalysisGraphDecoderImpl {
    ...
    /**
     * Determine whether the next callee on the target callpath. It is possible that the actual path
     * taken overlaps multiple target paths, resulting in a single long inlining chain.
     */
    ...
    private static int comparePaths(TargetPath targetPath, List<String> actualPath) {
        ...
        while (targetIdx < targetPath.size() && actualIdx < actualPath.size()) {
            if (actualPath.get(actualIdx).equals(targetPath.getMethodId(targetIdx))) {
                ...
                if (targetPath.getCallsite() != null && targetIdx == 0 && (actualIdx == 0 || !targetPath.getCallsite().getMethodId().equals(actualPath.get(actualIdx - 1)))) {
                    // Callsite doesn't match
                    actualIdx++;
                    continue;
                }
                actualIdx++;
    ...
    private boolean isBeyondCutoff(ResolvedJavaMethod method, PEMethodScope caller) {
        ...
        if (callerScope.isBeyondCutoff) {//Todo maybe allow for node size threshold conditions
            System.out.println("found transitive cutoff: "+ calleeId);
            return true;
        }

        /*
         * At this point, the callee being evaluated is not a root and is not past an
         * encountered cutoff. Determine whether the callee itself is a cutoff.
         */
        for (TargetPath cutoff : cutoffs) {
