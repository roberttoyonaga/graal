...
public class TestBasic {
    ...
    /** This both initializes NMT and does some basic checks to verify pre-init allocations are handled.*/
    @BeforeClass
    public static void setup() {
        ...
        // Reallocate a block that has lived across the initialization boundary. Verify its been recorded.
    ...
    @Test
    public void testReserveAndCommit() throws Throwable {
        assertTrue("Test should start with no memory already allocated in the mtTest category.", NativeMemoryTracking.getReservedByCategory(NmtFlag.mtTest) == 0);
        Pointer reservePtr = VirtualMemoryProvider.get().reserve(WordFactory.unsigned(RESERVE_SIZE), VirtualMemoryProvider.get().getGranularity(), false,
                        NmtFlag.mtTest.ordinal());
        assertTrue(NativeMemoryTracking.getReservedByCategory(NmtFlag.mtTest) >= RESERVE_SIZE);
        assertTrue(NativeMemoryTracking.getCommittedByCategory(NmtFlag.mtTest) == 0);

        Pointer commitPtr = VirtualMemoryProvider.get().commit(reservePtr, WordFactory.unsigned(COMMIT_SIZE), 0, NmtFlag.mtTest.ordinal());

        assertTrue(NativeMemoryTracking.getReservedByCategory(NmtFlag.mtTest) >= RESERVE_SIZE);
        assertTrue("Expected committed size:" + COMMIT_SIZE + " Actual:" + NativeMemoryTracking.getCommittedByCategory(NmtFlag.mtTest),
                        NativeMemoryTracking.getCommittedByCategory(NmtFlag.mtTest) >= COMMIT_SIZE);

        assertTrue("uncommit op failed", 0 == VirtualMemoryProvider.get().uncommit(commitPtr, WordFactory.unsigned(COMMIT_SIZE), NmtFlag.mtTest.ordinal()));
        assertTrue(NativeMemoryTracking.getCommittedByCategory(NmtFlag.mtTest) == 0);

        assertTrue("free op failed", 0 == VirtualMemoryProvider.get().free(reservePtr, WordFactory.unsigned(RESERVE_SIZE), NmtFlag.mtTest.ordinal()));

        assertTrue("After freeing memory for test, mtTest category should have size 0. Actual:" + NativeMemoryTracking.getReservedByCategory(NmtFlag.mtTest),
                        NativeMemoryTracking.getReservedByCategory(NmtFlag.mtTest) == 0);
    }
